<?php
class actions_send_email_productionheader {
    ...
    function handle(&$params) {
		...
		$message1 .= '<th>Downtime (minutes)</th>';
		$message1 .= '<th>Comments</th></tr>';
	
		foreach ($relatedRecords as $foobar){
			$message1 .= '<tr><td style="background: #FEE5DF">' . $foobar['cell'] . '</td>';
			$message1 .= '<td style="background: #FEE5DF">' . $foobar['machine'] . '</td>';
			$message1 .= '<td style="background: #FEE5DF">' . $foobar['part'] . '</td>';
			...
			$message1 .= '<td style="background: #FEE5DF">' . $foobar['down_time_min'] . '</td>';
